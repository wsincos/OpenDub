...
**已发布展示版本：** `v2.0.0-showcase`（V2）；`v0.0.1-alpha.0`（V1 基线）<br>
**当前工作树：** `v2.0.0-showcase`（已冻结并完成远端发布验证）

## 1. 一句话结论

OpenDub 的已发布 V1 基线完成了申报版的 **P0-P3 交付闭环**。V2 正式展示版本进一步完成了任务优先的交互展示：它可以清楚解释视频配音生成任务、可视化视频/文本/参考声音如何进入一套完整方法、展示可追溯的历史研究样例、交互式展示并选择三套完整研究方法、将选择和授权输入保存为本地项目，并导出可复现的准备记录。V2 申报演示视频、双语字幕、校验和、受控来源登记和交付清单已随仓库提供，并已通过第 4 轮独立严格审查（`9.1 / 10`）。
...
- 每个 case manifest 包含来源路径、SHA-256、项目负责人确认的 V2 再分发记录、方法标识和内容状态；受限原始素材另由 [受控来源登记](rights/showcase-source-evidence-v2.md) 记录哈希、许可范围、核验通道和撤回处理；
...
- [V2 申报视频交付](grant/video/v2/README.md)：`84.203` 秒真实浏览器交互录制、案例媒体事实边界、双语字幕、校验和与重建脚本；
