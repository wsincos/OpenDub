...
import { OpenDubCoverPage } from "../features/cover/OpenDubCoverPage";
...
import { OpenDubSummaryPage } from "../features/summary/OpenDubSummaryPage";
...
export function OpenDubApp() {
  ...
  const coverRoute = location.pathname === "/cover";

  return (
    <div className="atlas-app">
      {!coverRoute ? <header className="atlas-nav">
        <NavLink className="atlas-brand" to="/cover"><span>OD</span><strong>OpenDub</strong><small>METHOD ATLAS</small></NavLink>
        ...
        <nav className={menuOpen ? "is-open" : ""} aria-label="OpenDub primary navigation">
          ...
          <NavItem label="Summary" to="/summary" onNavigate={() => setMenuOpen(false)} />
        </nav>
      ...
      <Routes>
        ...
        <Route element={<OpenDubCoverPage />} path="/cover" />
        <Route element={<VttsTaskStagePage />} path="/vtts" />
        <Route element={<Navigate replace to="/vtts" />} path="/explore" />
        ...
        <Route element={<OpenDubSummaryPage />} path="/summary" />
        <Route element={<Navigate replace to="/vtts" />} path="*" />
      </Routes>
    </div>
  );
