...
ContentMode = Literal["concept", "replay", "live", "planned"]
RuntimeStatus = Literal["unavailable", "experimental", "stable"]
CoreMethodId = Literal[
...
class MethodSelection(BaseModel):
    """An evidence-bound choice of one complete method for a local project."""

    model_config = ConfigDict(extra="forbid", frozen=True)

    method_id: CoreMethodId
    method_manifest_version: str = Field(min_length=1, max_length=200)
    declared_need: str = Field(min_length=1, max_length=500)
    required_inputs: tuple[str, ...] = Field(min_length=1, max_length=12)
    optional_controls: tuple[str, ...] = Field(max_length=12)
    runtime_status: RuntimeStatus
    content_modes: tuple[ContentMode, ...] = Field(min_length=1, max_length=4)
...
class Project(BaseModel):
    ...
    input_authorizations: tuple[InputAuthorization, ...] = ()
    ...
    def select_method(self, selection: MethodSelection, expected_revision: int) -> Project:
        """Persist one complete-method choice using the project revision protocol."""
        if expected_revision != self.revision:
            raise DomainError(
                code="PROJECT_CONFLICT",
                message="Project was changed by another operation.",
    ...
    def authorize_input(self, authorization: InputAuthorization, expected_revision: int) -> Project:
        """Record an explicit authorization for the current video or target text input."""
        if expected_revision != self.revision:
            raise DomainError(
                code="PROJECT_CONFLICT",
                message="Project was changed by another operation.",
        ...
        if authorization.input_kind == "video":
            asset = next((item for item in self.assets if item.id == authorization.asset_id), None)
            if asset is None or asset.kind != "video":
                raise DomainError(
                    code="ASSET_NOT_FOUND",
                    message="Video authorization must reference a video asset in this project.",
                )
            if asset.sha256 != authorization.content_sha256:
                raise DomainError(
                    code="INPUT_INVALID",
                    message="Video authorization must match the declared asset hash.",
                )
        elif self.segments:
            # The text hash itself is generated by the local preparation workflow.
            pass
        else:
            raise DomainError(
                code="INPUT_INVALID",
                message="Add target text before recording its authorization.",
            )
        return self.model_copy(
            update={
                "input_authorizations": (*self.input_authorizations, authorization),
                "revision": self.revision + 1,
                "updated_at": datetime.now(UTC),
            }
        )
